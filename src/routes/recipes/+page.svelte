			...
			{#each data.recipes as recipe}
				...
				>
					...
					<div class="flex flex-wrap gap-2 text-sm text-[var(--color-deep)] sm:justify-end">
						<span class="rounded-md app-badge px-2 py-1">{recipe.servings} servings</span>
						<span class="rounded-md app-badge px-2 py-1">{recipe.ingredientCount} ingredients</span>
						{#if recipe.caloriesPerServing !== null}
							<span class="rounded-md app-badge px-2 py-1">{recipe.caloriesPerServing} kcal</span>
						{/if}
						{#if recipe.proteinPerServing !== null}
							<span class="rounded-md app-badge-alt px-2 py-1">{recipe.proteinPerServing}g protein</span>
						{/if}
					</div>
				</a>
			{/each}
